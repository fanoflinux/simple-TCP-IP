...
int main(int argc,char *argv[])
{
    ...
    ep_enevnts=malloc(sizeof(struct epoll_event)*EPOLL_SIZE);
    event.events=EPOLLIN;
    event.data.fd=serv_sock;
    epoll_ctl(epfd,EPOLL_CTL_ADD,serv_sock,&event);
    while(1)
    {
        event_cnt=epoll_wait(epfd,ep_enevnts,EPOLL_SIZE,-1);
        if(event_cnt==-1)
        {
        ...
        {
            if(ep_enevnts[i].data.fd==serv_sock)
            {
            ...
            {
                str_len=read(ep_enevnts[i].data.fd,buf,BUF_SIZE);
                if(str_len==0)
                {
                    epoll_ctl(epfd,EPOLL_CTL_DEL,ep_enevnts[i].data.fd,NULL);
                    close(ep_enevnts[i].data.fd);
                    printf("closed client:%d\n",ep_enevnts[i].data.fd);
                }
                else
                {
                    write(ep_enevnts[i].data.fd,buf,BUF_SIZE);//回声操作
                }
            }
        }
    }
